    ...
    <>
      <header className="sticky top-0 z-40 bg-[var(--background)]/80 backdrop-blur-md">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center gap-3 sm:gap-4">
          {/* 좌측: 뒤로가기 + 로고 (lg 미만에서 표시 — rail 이 보이는 lg+ 에선 rail 상단이 로고 담당) */}
          <div className="flex items-center gap-2 flex-shrink-0">
            {!isTabRoot && (
              <button
                onClick={() => router.back()}
                className="lg:hidden p-1 -ml-1 text-[var(--foreground)]"
                aria-label="뒤로가기"
              >
            ...
            <Link href={`/${locale}`} className="lg:hidden flex items-center flex-shrink-0" aria-label="홈으로">
