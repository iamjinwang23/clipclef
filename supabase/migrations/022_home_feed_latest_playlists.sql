-- Migration 022: home_feed 전체 피드 정렬을 인기순 → 최신순으로 변경
-- 기존: top_playlists ORDER BY like_count DESC
-- 변경: top_playlists ORDER BY created_at DESC
-- 인덱스 idx_playlists_active_created (019에서 이미 생성)가 새 정렬을 지원.
...
AS $$
  ...
  -- ─── 플레이리스트 Top N (created_at desc — 최신순) ──────────────────────
  top_playlists AS (
  ...
  SELECT jsonb_build_object(
    ...
    'playlists', COALESCE((SELECT jsonb_agg(to_jsonb(tp) ORDER BY tp.created_at DESC) FROM top_playlists tp), '[]'::jsonb)
  );
$$;
